...
export class SubirFotoOrdenComponent implements OnInit {
  ...
  constructor(private service: OrdenVentaService,
    ...
    private multimediaService: MultimediaService) {
      this.route.paramMap.subscribe(params => {
        ...
        if(id){
          this.service.ver(id).subscribe(orden => {
            this.orden = orden;
            this.titulo = `Subir foto de órden de ${this.orden.paciente.nombreCompleto}`;
            this.cargarFotos();
          });
        }
      });
  ...
  cargarFotos(): void {
    this.multimediaService.buscarPorOrdenVentaId(this.orden.id).subscribe(multimedia => {
  ...
  eliminar(foto: Multimedia){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.multimediaService.eliminar(foto.id).subscribe(res => {
